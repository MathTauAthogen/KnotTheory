...
mathematica("Partitions = " + str(list(test3[0])).replace("[", "{").replace("]", "}"))
mathematica("Paths = " + str(occurences).replace("[", "{").replace("]", "}"))
mathematica("""(*End second boilerplate cell*)
...
mathematica("Proj = DirSum[" + str(list(map(int, projmatr))).replace("[", "{").replace("]", "}") + "]\n")
mathematica("ComputePoly[c_] := (m = Proj.DMatrix; Do[m = m.elem, {elem, c}]; Return[Tr[m]])")
...
for i in range(0, len(test2)):
    ...
    mathematica("R" + str(i + 1) + " = SumMatrs[{" + str(finalOutput) + "}]")
    mathematica("")
